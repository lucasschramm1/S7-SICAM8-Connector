...
void readCallback(UA_Client *client, void *userdata, UA_UInt32 requestId, UA_ReadResponse *response) 
{
    ...
    {
        auto S7toSICAM8Nodes = static_cast<std::map<std::string, std::pair<int, int>>*>(userdata);
...
int main(int argc, char** argv)
{
    ...
    // Deklaration eines Open62541 Clients inklusive Default-Config, Timeout von 60 s und SCL von 1h
    ...
    while (true)
    {
        ...
        while (verbunden)
        {
            ...
            if (statusCode != UA_STATUSCODE_GOOD)
            {
                ...
                verbunden = false;
                UA_Client_disconnect(client);
                // Beende die Schleife, wenn der Client nicht mehr verbunden ist
                break;
            }
            ...
            {
                // Erstellen der Leseanfragen für die Elemente aus S7 zu SICAM8 Nodes
                ...
                for (const auto& entry : S7toSICAM8Nodes) 
                {
                    ...
                    int namespaceIndex = entry.second.second;

                    UA_ReadValueId rv;
                    UA_ReadValueId_init(&rv);
                    rv.nodeId = UA_NODEID_NUMERIC(namespaceIndex, nodeId);
                    rv.attributeId = UA_ATTRIBUTEID_VALUE;
                    readValues.push_back(rv);
                }
                // Asynchrone Leseanfragen senden
                if (!readValues.empty()) 
                {
                    readDone = false;
