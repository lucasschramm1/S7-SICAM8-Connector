...
bool secureChannelRenewalInProgress = false;
...
processOPNResponse(UA_Client *client, const UA_ByteString *message) {
     ...
     * DateTime=Int64 we take 75% now 70% of lifetime to start renewing as described in
     * standard */
    client->nextChannelRenewal = UA_DateTime_nowMonotonic()
            + (UA_DateTime) (response.securityToken.revisedLifetime
                    * (UA_Double) UA_DATETIME_MSEC * 0.2);
    ...
    secureChannelRenewalInProgress = false;
...
sendOPNAsync(UA_Client *client, UA_Boolean renew) {
    ...
    secureChannelRenewalInProgress = true;
