    ...
    while (true)
    {
        ...
        while (!verbunden)
        {
            ...
            if (status == UA_STATUSCODE_GOOD)
            {
                ...
                request.requestedLifetimeCount = 60000; // Lebensdauer
                request.requestedMaxKeepAliveCount = 50; // Maximale Anzahl von Keep-Alive
                ...
                if (response.responseHeader.serviceResult == UA_STATUSCODE_GOOD)
                {
                    ...
                    for (const auto& signal : readSignals)
                    {
                        ...
                        UA_MonitoredItemCreateResult monResponse = UA_Client_MonitoredItems_createDataChange(client, subscriptionId, UA_TIMESTAMPSTORETURN_BOTH, monRequest, (void*)signal.name.c_str(), dataChangeNotificationCallback, NULL);
